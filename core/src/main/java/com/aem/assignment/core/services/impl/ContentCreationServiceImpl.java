...
public class ContentCreationServiceImpl implements ContentCreationService {
    ...
    private void createPages(ResourceResolver resolver, JSONObject contentJson) throws WCMException, PersistenceException, JSONException, ReplicationException, RepositoryException {
        ...
        try {
            ...
            log.error("JSON error while creating page: ", e);
        } catch (WCMException e) {
            log.error("WCM error while creating page: ", e);
        } catch (PersistenceException e) {
            log.error("Persistence error while creating page: ", e);
        } catch (ReplicationException e) {
            log.error("Replication error while creating page: ", e);
        } catch (RepositoryException e) {
            log.error("Repository error while creating page: ", e);
        } finally {
            if (resolver != null && resolver.isLive()) {
                resolver.close();
            }
